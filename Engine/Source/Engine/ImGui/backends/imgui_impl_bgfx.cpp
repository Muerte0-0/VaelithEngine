...
// Data
static bgfx::ViewId g_View = 255;
static bgfx::ViewId g_Views = 255;
...
// Viewports whose framebuffers have not been created yet because
// bgfx::createFrameBuffer(nwh) may only be called between frames.
// They are flushed via ImGui_Implbgfx_FlushWindowOps() right after bgfx::frame().
struct PendingWindowOp
{
    ImGuiViewport* viewport = nullptr;
    uint16_t       w = 0;
    uint16_t       h = 0;
    bool           destroy = false; // true = destroy old fb before (re)create
};
static std::vector<PendingWindowOp> g_PendingWindowOps;
...
static void uploadRect(ImTextureData* texture, const int x, const int y, const int w, const int h)
{
    ...
    //const bgfx::Memory* mem = bgfx::copy(src, texture->GetPitch() * h); todo use this
...
static void ImGui_Implbgfx_CreateWindow(ImGuiViewport* viewport) noexcept
{
    ...
    vd->viewId = g_Views--;  // count down: 254, 253, 252 ... away from main passes
    viewport->RendererUserData = vd;

    g_PendingWindowOps.push_back({
        viewport,
        static_cast<uint16_t>(viewport->Size.x),
        static_cast<uint16_t>(viewport->Size.y),
        false
    });
}
...
{
    // Remove any pending ops for this viewport so we don't touch freed memory
    g_PendingWindowOps.erase(
        std::remove_if(g_PendingWindowOps.begin(), g_PendingWindowOps.end(),
            [viewport](const PendingWindowOp& op){ return op.viewport == viewport; }),
        g_PendingWindowOps.end()
    );
...
static void ImGui_Implbgfx_RenderWindow(ImGuiViewport* viewport, void*) noexcept
{
    ...
    // fb not ready yet (pending flush after bgfx::frame()) — skip this frame
    if (!vd || !bgfx::isValid(vd->fb))
        return;
    ...
    bgfx::setViewFrameBuffer(vd->viewId, vd->fb);
    bgfx::setViewRect(vd->viewId, 0, 0, w, h);
    bgfx::setViewClear(vd->viewId, BGFX_CLEAR_COLOR | BGFX_CLEAR_DEPTH, 0x000000ff, 1.0f, 0);
    bgfx::touch(vd->viewId);
...
{
    // Queue a destroy+recreate — must not call bgfx::createFrameBuffer mid-frame
    g_PendingWindowOps.push_back({
        viewport,
        static_cast<uint16_t>(size.x),
        static_cast<uint16_t>(size.y),
        true  // destroy existing fb first
    });
}
...
{
    g_View  = static_cast<bgfx::ViewId>(view);
    // Secondary viewport windows count DOWN from 254 so they never collide
    // with the main UI pass (255) or any other forward-allocated view IDs.
    g_Views = 254;
...
void ImGui_Implbgfx_NewFrame()
{
    ...
    // Re-register clear flags for every active secondary viewport each frame.
    // bgfx resets view state after bgfx::frame(), so setViewClear must be
    // called again every frame or the framebuffer will ghost.
    for (ImGuiViewport* vp : ImGui::GetPlatformIO().Viewports)
    {
        if (vp->Flags & ImGuiViewportFlags_IsPlatformWindow)
            continue;

        const auto* vd = static_cast<ImGuiBGFXViewportData*>(vp->RendererUserData);
        if (vd && bgfx::isValid(vd->fb))
            bgfx::setViewClear(vd->viewId, BGFX_CLEAR_COLOR | BGFX_CLEAR_DEPTH, 0x000000ff, 1.0f, 0);
    }
}

void ImGui_Implbgfx_FlushWindowOps()
{
    // Safe to call bgfx::createFrameBuffer(nwh) here — we are between frames,
    // i.e. this is called immediately after bgfx::frame().
    for (const PendingWindowOp& op : g_PendingWindowOps)
    {
        auto* vd = static_cast<ImGuiBGFXViewportData*>(op.viewport->RendererUserData);
        if (!vd)
            continue;

        if (op.destroy && bgfx::isValid(vd->fb))
        {
            bgfx::destroy(vd->fb);
            vd->fb = BGFX_INVALID_HANDLE;
        }

        vd->fb = bgfx::createFrameBuffer(
            op.viewport->PlatformHandleRaw,
            op.w, op.h,
            bgfx::TextureFormat::RGBA8
        );
    }
    g_PendingWindowOps.clear();
}